...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg && msg.type === 'JIMSCANNER_FETCH_ORDERS') {
    handleFetchOrders()
      .then((result) => sendResponse({ ok: true, ...result }))
      .catch((err) => sendResponse({ ok: false, error: String(err && err.message ? err.message : err) }))
    return true
  }
  if (msg && msg.type === 'JIMSCANNER_ADD_AS_MY_ADDRESS') {
    handleAddAsMyAddress(msg.sourceAddressId)
      .then((result) => sendResponse({ ok: true, ...result }))
      .catch((err) => sendResponse({ ok: false, error: String(err && err.message ? err.message : err) }))
    return true
  }
...
async function handleAddAsMyAddress(sourceAddressId) {
  const { apiUrl, token } = await chrome.storage.local.get(['apiUrl', 'token'])
  if (!token) throw new Error('토큰이 설정되지 않았습니다.')
  const base = (apiUrl || DEFAULT_API_URL).replace(/\/$/, '')
  const url = base + '/api/extension/forwarder-addresses'
  let res
  try {
    res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
      body: JSON.stringify({ source_address_id: sourceAddressId }),
    })
  } catch (err) {
...
async function handleFetchOrders() {
  ...
  if (!token) throw new Error('토큰이 설정되지 않았습니다.')
  const base = (apiUrl || DEFAULT_API_URL).replace(/\/$/, '')
  const url = base + '/api/extension/orders'
  let res
  try {
