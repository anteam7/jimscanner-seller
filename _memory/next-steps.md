...
## 🆕 보류 (트리거 대기) — 쿠팡 발송처리 자동 반영

**결정 (2026-06-10):** 짐스캐너에서 운송장 등록 시 쿠팡 Wing에 발송처리(송장 등록)를 자동 push 하는 연동을 **구축하기로 함**.
- **현재 상태:** 기능 없음. 짐스캐너 → 쿠팡 방향 연동은 전무 (상태 변경·운송장 등록은 우리 DB `b2b_orders.status`만 갱신). 카카오 알림톡(구매자 대상)만 외부로 나감.
- **트리거:** 사용자가 "다음에 등록해야 하는 운송장이 생기면 진행" 요청 → **실제 쿠팡 주문 1건으로 e2e 검증하며 구축**. 운송장 등록 case 발생 시 이 항목 착수.
- **선결:** 쿠팡 OpenAPI(판매자센터) 인증키 발급 — Access Key / Secret Key / Vendor ID. env 미등록.
- **범위 메모:** v1급 신규. 스마트스토어·옥션 등 다른 마켓은 별도. 발송처리 API = 쿠팡 "주문 출고(송장) 등록" 엔드포인트.
...
보류 사유: 사용자 컴퓨터 앞에 앉을 시간 부족. 계속 자율 작업 진행.
...
| **main repo 어드민 `/admin/forwarders` + `/admin/centers` 에 `default_phone` 입력 필드 추가** | `forwarders.default_phone` + `centers.default_phone` 컬럼 (2026-05-20 추가). 시드는 `COALESCE(c.default_phone, f.default_phone)` 우선. 공개 phone 36/57 자동 수집 완료 (US 30/40, JP 6/17). 남은 21개 (호야USA·훗타운 JP/DE·비드팟·유니옥션·엘덱스 JP 등) 는 어드민에서 셀러 로그인 정보 보고 채워야 함 |
