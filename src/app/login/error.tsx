...
/**
 * 로그인 에러 바운더리 — /login 클라이언트 컴포넌트의 렌더 throw
 * (Supabase 세션 확인·리다이렉트 처리 실패 등) 를 잡아 루트 레이아웃을
 * 갈아치우지 않고 미인증 맥락에 맞는 복구 안내를 보여준다. global-error 폴백은
 * 루트 전체를 대체하고 "대시보드로" 만 안내해 로그인 전 사용자에겐 부적합하므로,
 * 이 세그먼트 전용 그물을 둔다. 정상 경로에선 렌더되지 않음 (순수 additive).
 */
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 text-slate-900">
      ...
      >
        ...
        <h1 className="text-lg font-bold tracking-tight text-slate-900">로그인 중 문제가 발생했습니다</h1>
        <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-600">
          로그인 화면을 불러오는 중 일시적인 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.
          문제가 계속되면 아직 계정이 없으신 경우 회원가입으로 진행할 수 있습니다.
        </p>
