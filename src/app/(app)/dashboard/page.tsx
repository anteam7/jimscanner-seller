...
function StatusBanner({ account }: { account: FullAccount }) {
  if (account.suspended_at) {
    return (
      <div className="rounded-xl border border-red-800 bg-red-950/30 px-5 py-4">
        <p className="text-sm font-semibold text-red-300">계정이 정지되었습니다</p>
  ...
  if (account.verification_level >= 3) {
    return (
      <div className="rounded-xl border border-indigo-800/60 bg-indigo-950/20 px-5 py-4 flex items-center gap-3">
        <div className="w-9 h-9 rounded-full bg-indigo-900/60 border border-indigo-700/50 flex items-center justify-center flex-shrink-0">
  ...
  return (
    <div className="rounded-xl border border-amber-800/50 bg-amber-950/20 px-5 py-4">
      <p className="text-sm font-semibold text-amber-300">계정 검증 진행 중</p>
      <p className="text-xs text-slate-400 mt-1">
        다음 단계:{' '}
        <span className="text-amber-200">{next?.label}</span>
        {next?.hint ? ` — ${next.hint}` : ''}
      </p>
      {next?.href && (
        <a
          href={next.href}
          className="inline-block mt-2.5 text-xs font-medium text-amber-300 hover:text-amber-200 underline underline-offset-2 transition-colors"
        >
          지금 완료하기 →
        </a>
      )}
...
function StatSkeleton({ label }: { label: string }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      ...
      <div className="h-7 w-16 bg-slate-800 rounded-md animate-pulse" />
      <div className="h-3 w-24 bg-slate-800/60 rounded-md mt-2 animate-pulse" />
    </div>
  )
}
