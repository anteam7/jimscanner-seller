    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-white">구독 취소</h1>
        ...
        {step === 'select_reason' && (
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 space-y-4">
            <p className="text-sm font-medium text-slate-300">취소 이유 선택 <span className="text-red-400">*</span></p>
            ...
            {reason === '기타' && (
              <div className="space-y-1">
                <textarea
                  ...
                  rows={3}
                  maxLength={500}
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none"
                />
                <p className={`text-right text-xs ${detail.length >= 500 ? 'text-red-400' : 'text-slate-500'}`}>
                  {detail.length}/500
                </p>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-lg border border-slate-600 text-sm font-medium text-slate-300 hover:bg-slate-800 transition-colors"
              >
                취소하지 않기
              </button>
              <button
        ...
        {/* Step: 오퍼 모달 */}
        {step === 'offer' && (
          <div className="bg-slate-900 border border-indigo-600 rounded-2xl p-6 space-y-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-indigo-900/60 flex items-center justify-center shrink-0">
              ...
              <div>
                <p className="text-base font-semibold text-white">잠깐, 특별 혜택을 드릴게요</p>
                <p className="text-xs text-slate-400 mt-0.5">지금 취소를 보류하시면 아래 혜택을 드립니다</p>
              </div>
            </div>

            <div className="bg-indigo-950/50 border border-indigo-700/50 rounded-xl px-5 py-4 space-y-2">
              <div className="flex items-center gap-2">
                <span className="inline-block w-2 h-2 rounded-full bg-indigo-400" />
                <span className="text-sm font-semibold text-indigo-200">3개월간 30% 요금 할인</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-block w-2 h-2 rounded-full bg-indigo-400" />
                <span className="text-sm text-slate-300">구독 기간 3개월 연장</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-block w-2 h-2 rounded-full bg-indigo-400" />
                <span className="text-sm text-slate-300">모든 기능 그대로 유지</span>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1">
              ...
              <button
                ...
                disabled={loading}
                className="w-full px-4 py-2 rounded-lg text-sm text-slate-500 hover:text-slate-300 transition-colors"
              >
                혜택 없이 취소 진행
              </button>
              <button
                ...
                disabled={loading}
                className="w-full px-4 py-2 rounded-lg text-sm text-slate-600 hover:text-slate-400 transition-colors"
              >
                ← 이유 다시 선택
              </button>
            </div>
        ...
        {step === 'confirm' && (
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 space-y-5">
            <div className="text-center space-y-2">
              <p className="text-base font-semibold text-white">정말 취소하시겠습니까?</p>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 px-4 py-2.5 rounded-lg border border-slate-600 text-sm font-medium text-slate-300 hover:bg-slate-800 disabled:opacity-50 transition-colors"
              >
                이전
              </button>
              <button
        ...
        {/* Step: 완료 */}
        {step === 'done' && (
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 text-center space-y-5">
            <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-slate-800">
              <svg className="w-7 h-7 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <div>
              <p className="text-lg font-semibold text-white">구독이 취소되었습니다</p>
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-sm font-medium text-white transition-colors"
            >
              대시보드로 이동
            </button>
          </div>
