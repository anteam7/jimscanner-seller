      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 rounded-full bg-indigo-900/50 border border-indigo-700 flex items-center justify-center">
          ...
          {email && (
            <p className="text-indigo-300 font-medium text-center text-sm mb-6 break-all">
              {email}
            </p>
          )}

          {/* 안내 박스 */}
          <div className="rounded-lg border border-slate-700 bg-slate-900/40 px-4 py-4 mb-6 space-y-2">
            <p className="text-sm font-medium text-slate-200">인증 방법</p>
            <ol className="text-sm text-slate-400 space-y-1 list-decimal list-inside">
              <li>받은 편지함(또는 스팸함)에서 짐스캐너 이메일을 찾으세요.</li>
              <li><span className="text-white font-medium">&apos;이메일 인증하기&apos;</span> 버튼을 클릭하세요.</li>
          ...
          {/* 성공 메시지 */}
          {resendMsg && (
            <div className="rounded-lg bg-green-950/60 border border-green-800 px-4 py-3 text-sm text-green-300 mb-4">
              {resendMsg}
            </div>
          )}
          ...
          {/* 오류 메시지 */}
          {error && (
            <div className="rounded-lg bg-red-950/60 border border-red-800 px-4 py-3 text-sm text-red-300 mb-4">
              {error}
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              className="w-full border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white h-10 disabled:opacity-50"
            >
              {resending
                ? '발송 중…'
                : resendCooldown > 0
