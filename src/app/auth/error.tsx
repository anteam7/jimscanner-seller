...
/**
 * 인증 플로우 에러 바운더리 — /auth 하위(콜백·비밀번호 재설정·MFA 챌린지)
 * 클라이언트 컴포넌트의 렌더 throw (Supabase 세션·토큰 교환 실패 등) 를 잡아
 * 루트 레이아웃을 갈아치우지 않고 미인증 맥락에 맞는 복구 안내를 보여준다.
 * global-error 폴백은 루트 전체를 대체하고 "대시보드로" 만 안내해
 * 로그인 전 사용자에겐 부적합하므로, 이 세그먼트 전용 그물을 둔다.
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 text-slate-900">
      ...
      >
        ...
        <h1 className="text-lg font-bold tracking-tight text-slate-900">인증 처리 중 문제가 발생했습니다</h1>
        <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-600">
          인증 링크를 처리하는 중 일시적인 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.
          비밀번호 재설정·인증 메일 링크가 만료되었다면 로그인 화면에서 다시 요청할 수 있습니다.
        </p>
