...
/** 계정 컬럼/입력값을 안전한 무료 보관일로 파싱 (1~60일, 범위 밖·null 이면 기본값). */
export function parseFreeStorageDays(raw: string | number | null | undefined): number {
  if (raw == null) return DEFAULT_FREE_STORAGE_DAYS
  const n = Number(raw)
